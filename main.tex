...
% DISCENTE: FABRICIO DOS SANTOS ASSUNÇÃO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\author{Mário Diego Rocha Valente\inst{1}, Roberto Samarone dos Santos\inst{2}}

\address{Estatístico, Analista de Trânsito(DETRAN-PA) \\ Graduando em Sistemas de Informação - Universidade Federal do Pará(UFPA) \\
  Rua Augusto Corrêa, 01 - Guamá - Belém - PA - Brasil
\nextinstitute
  Professor Dr., Faculdade de Computação - Universidade Federal do Pará(UFPA)\\
  Rua Augusto Corrêa, 01 - Guamá - Belém - PA - Brasil \\
  \email{mario.vatente@detran.pa.gov.br, rsa@ufpa.br}
  }
...
A aplicação do algoritmo de Edmonds--Karp à rede definida na Fig.~\ref{fig:grafo}
resultou na identificação de diversos caminhos aumentantes, com seus respectivos
gargalos (fluxos possíveis em cada iteração) e atualização do fluxo acumulado
$\lvert f \rvert$. A cada iteração, o algoritmo utilizou uma busca em largura
(BFS) para encontrar o caminho mais curto em número de arestas entre a fonte $S$
e o sorvedouro $T$. Em seguida, o gargalo $\Delta$ foi calculado como a menor
capacidade residual ao longo do caminho. Por fim, as capacidades residuais foram
atualizadas e o valor de $\lvert f \rvert$ ajustado.
